...
checkout() {
    ...
    if ! is_repo "${PI_HOLE_FILES_DIR}" ; then
        ...
        echo -e "  Please re-run install script from https://github.com/craeckor/pi-hole${COL_NC}"
        exit 1;
    fi
    ...
    if [[ "${1}" == "dev" ]] ; then
        ...
        if check_download_exists "$path"; then
            ...
            ftlbranches=( $(git ls-remote https://github.com/craeckor/FTL | grep 'heads' | sed 's/refs\/heads\///;s/ //g' | awk '{print $2}') )
